...
export default function DateSelect ({ items, onChange, showValueInside = true }) {
  return (
    <div className='
      flex flex-row items-center justify-between gap-0 m-0 pl-2 flex-nowrap whitespace-nowrap
    ...
    '
    >
      <IoCalendarOutline aria-disabled aria-label='Calendar icon' className='h-5 w-5 text-neutral-800 m-0' />
      <div className='flex flex-row gap-0 items-center justify-center w-fit'>
        <BaseSelect onChange={onChange} defaultValue={items[0]} items={items} label='FechaInicio' showValueInside={showValueInside} />
        <PiArrowsHorizontalBold aria-disabled aria-label='Arrows left right icon' className='h-fit w-5 text-neutral-600' />
